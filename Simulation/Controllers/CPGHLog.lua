...
if (sim_call_type==sim_childscriptcall_initialization) then
    ...
    require("lua/modular/logger")
    json = require("lua/dkjson")
    sensor = {}
    ...
    filelog = io.open("/home/rodr/Desktop/log"..suffix..".json", "a")
    --fileHorm = io.open("/home/rodr/Desktop/Hormlog"..suffix..".json", "a")
    filelog:write("[")
    inittime = true
    --tstep = simGetSimulationTime()
...
if (sim_call_type==sim_childscriptcall_actuation) then
    tstep = simGetSimulationTime()
    inittime = logtimeopen(tstep,filelog,inittime)
    ...
    logHorm(horm,filelog)
    logHCount(count,filelog)
    logHCountE(countE,filelog)
    count = {0,0,0,0,0}
    countE = {0,0,0,0,0}
    ...
    --[[if(horm[1]==-1) then
        sendhorm = true
        ampd,offd,phasediff,v = receptors(horm,ampd,offd,phasediff,v,ampset,offsetset,phasediffset,vset,delta)
    else
        sendhorm = false
        if(r>0.75) then
            ampd,offd,phasediff,v = receptors(horm,ampd,offd,phasediff,v,ampset,offsetset,phasediffset,vset,delta)            
    ...
    ampd,offd,phasediff,v = receptors(horm,ampd,offd,phasediff,v,ampset,offsetset,phasediffset,vset,delta)
    horm[1] = -1
    logSetP(ampd,offd,phasediff,v,filelog)
    ...
    logCPG(teta,ampli,dampli,offset,doffset,filelog)
                    ...
                    if (connh[j]~=-1 and #phorm[j]>0) then
                        for k=1,#phorm[j] do
                            if (propagate(proprob)) then
                                ...
                                receivtable[j][k][i] = true
                                --if(suffix==3) then
                                    --print('Propagated hormone received from '..j..' in '..k..' to '..i)
                                --end
                            end
                        end
                    end
...
if (sim_call_type==sim_childscriptcall_sensing) then
    ...
    while (data ~= nil) do
        ...
        countE = integrate(rhorm,countE)
        ampd,offd,phasediff,v = receptors(rhorm,ampd,offd,phasediff,v,ampset,offsetset,phasediffset,vset,delta)
        data,sender = simReceiveData(1038,'horm')
    end
   
    for k in pairs(receivtable) do
        for i in pairs(receivtable[k]) do
            receivtable[k][i]=nil
        end
    end

    receivtable = logRecH(phorm,filelog)
