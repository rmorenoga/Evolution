 ...
 */
public class VincentObjectiveFunction extends BenchmarkObjectiveFunction {
    ...
    public double evaluate(double[] values) {
        ...
        for (int i = 0; i < values.length; i++) {
            norm_values[i] = ((values[i] + 1.0)/2.0)*9.75 + 0.25;
        }
        
        double sum = 0.0;
        
        for (int i = 0; i < norm_values.length; i++) {
            sum += Math.sin(10.0*Math.log(norm_values[i]));
        }
        
        fitness = - (1.0/values.length)*sum + 0.9999999999999907;
