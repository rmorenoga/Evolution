 ...
 */
public class Deb1ObjectiveFunction extends BenchmarkObjectiveFunction {
    ...
    public double evaluate(double[] values) {
        ...
        for (int i = 0; i < norm_values.length; i++) {
            sum += Math.pow(Math.sin(5.0*Math.PI*norm_values[i]), 6.0);
        }
        
        fitness = -(1.0/values.length)*sum + 1.0;
