...
public class CFMSU9 extends ObjectiveFunction {
	...
	public double evaluate(double[] values) {
		...
		myRank = myRank + EvolJEAFMazeR9.startNumber;
		...
		for (int j = 0; j < maxTries; j++) {
			...
			if (clientID != -1) {
				...
				mazeseq = new char[] { 's', 'l', 's', 'b', 's', 'r', 's', 's', 'r', 's', 'b', 's', 'l', 's', 's', 'l', 's', 'b', 's', 'r', 's' };
				strSeq.setArray(mazeseq);
				vrep.simxSetStringSignal(clientID, "Maze", strSeq,
						vrep.simx_opmode_oneshot_wait);
...
//		}
	...
	void RestartSim(int myRank, int j) {
		...
		try {
			...
			qq = new ProcessBuilder(vrepcommand, "-h",
					"/home/rodr/EvolWork/Modular/Maze/MazeBuilderR02.ttt");
			// qq = new
